...
static int auralic_snd_soc_bind_card(struct snd_soc_card *card,int idx)
{
	struct snd_soc_pcm_runtime *rtd;
	struct snd_soc_component *component;
	struct snd_soc_dai_link *dai_link;
...
/**
 * auralic_snd_soc_register_card - Register a card with the ASoC core
 *
 * @card: Card to register
 *
 */
int auralic_snd_soc_register_card(struct snd_soc_card *card,int idx)
{
	if (!card->name || !card->dev)
		return -EINVAL;
	...
	return auralic_snd_soc_bind_card(card,idx);
}
EXPORT_SYMBOL_GPL(auralic_snd_soc_register_card);
				   ...
				   struct snd_soc_dai_link *dai_link)
{
	...
	for_each_link_codecs(dai_link, index, component) {
		...
		if (ret < 0)
			goto err;
        printk(KERN_ERR "the dai name is:%s\n",component->dai_name);
	}
	return 0;
err:
