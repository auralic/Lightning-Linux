...
static const unsigned int fsl_sai_rates[] = {
	...
	384000, 705600, 768000, 1411200, 2822400,5644800,
};
...
static int fsl_sai_set_bclk(struct snd_soc_dai *dai, bool tx, u32 freq)
{
	...
	for (; id < FSL_SAI_MCLK_MAX; id++) {
		...
		if (!clk_rate)
        {
      //      dev_err(dai->dev,"====no clk rate\n");
			continue;
        }
        //dev_err(dai->dev,"====in circle===%d===%ld\n",id,clk_rate);

        if(sai->is_mclk_input)
        {
            if(!(freq % 48000))
            {
                freq = (freq / 48000)*44100;
            }
        }
    ...
    if (sai->soc_data->max_register >= FSL_SAI_MCTL) {
        ...
        if(!sai->is_mclk_input)	
        {
            dev_err(dai->dev,"set to master mode....\n");
		...
		struct snd_soc_dai *cpu_dai)
{
	...
	if (sai->slot_width)
    {
        //dev_err(cpu_dai->dev,"CCCCCCC:%d\n",sai->slot_width);
		slot_width = sai->slot_width;
    }
	if (sai->slots)
    {
        //dev_err(cpu_dai->dev,"AAAAAA:%d\n",sai->slots);
		slots = sai->slots;
    }
	else if (sai->bclk_ratio)
    {
        //dev_err(cpu_dai->dev,"BBBB:%d==%d\n",sai->bclk_ratio,slot_width);
    ...
    //dev_err(cpu_dai->dev,"== in hw param function..===%d===%d===%d===%d===%d\n",pins,slots,slot_width,rate,bclk);
	if (!IS_ERR_OR_NULL(sai->pinctrl)) {
        //dev_err(cpu_dai->dev,"==1111111 in hw param function..\n");
	...
	if (!sai->is_dsp_mode)
    {
        //dev_err(cpu_dai->dev,"-=====is not dsp mode...\n");
	...
	if (sai->is_multi_lane) {
		if (tx) {
            ...
            //dev_err(cpu_dai->dev,"number is:%d==%d==%d===0x%x\n",sai->audio_config[tx].dst_fifo_off,sai->dma_params_tx.maxburst,sai->dma_params_tx.peripheral_size,dl_cfg[dl_cfg_idx].mask[tx]);
...
static int fsl_sai_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(np, "fsl,sai-mclk-direction-input", NULL) &&
	    sai->soc_data->max_register >= FSL_SAI_MCTL) {
