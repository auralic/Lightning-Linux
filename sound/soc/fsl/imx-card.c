	...
	if (!format_is_tdm(link_data)) {
		if (format_is_dsd(params)) {
			...
			fmt = (rtd->dai_link->dai_fmt & ~SND_SOC_DAIFMT_FORMAT_MASK) |
			      SND_SOC_DAIFMT_PDM;
            //auralic sai1 setting,dsd mode slave bclk,master frame sync
            fmt = (fmt & ~SND_SOC_DAIFMT_MASTER_MASK) | SND_SOC_DAIFMT_CBM_CFS;
		} else {
			slots = 2;
			slot_width = params_physical_width(params);
			fmt = (rtd->dai_link->dai_fmt & ~SND_SOC_DAIFMT_FORMAT_MASK) |
			      SND_SOC_DAIFMT_I2S;
            //auralic sai1 setting,pcm mode slave bclk and slave frame sync
            fmt = (fmt & ~SND_SOC_DAIFMT_MASTER_MASK) | SND_SOC_DAIFMT_CBM_CFM;
		}
	}
/*
...
*/
	for_each_rtd_codec_dais(rtd, i, codec_dai) {
        //dev_err(dev,"codec dai name is:%s\n",codec_dai->name);
        ...
        if(!strncmp(codec_dai->name,"es9028pro",strlen("es9028pro")))
        {
            if(format_is_dsd(params))
            {
        ...
        if(!strncmp(codec_dai->name,"llink_output",strlen("llink_output")))
        {
            if(format_is_dsd(params))
            {
                switch (params_rate(params))
        ...
        if(!strncmp(codec_dai->name,"llink_input",strlen("llink_input")) || !strncmp(codec_dai->name,"xmos_usbin",strlen("xmos_usbin")))
        {
            if(format_is_dsd(params))
            {
                switch (params_rate(params))
...
static int imx_card_probe(struct platform_device *pdev)
{
    ...
    if (of_property_read_u32(np, "auralic_sound_card_index",
                &card_index))
    {
    ...
    {
        ret = auralic_devm_snd_soc_register_card(&pdev->dev, &data->card,card_index);
        if (ret)
            return dev_err_probe(&pdev->dev, ret, "snd_soc_register_card failed\n");
    }
    return 0;
