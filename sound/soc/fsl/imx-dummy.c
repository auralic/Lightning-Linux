...
// Remove 44100. Let Android get 48kHz.
...
static int imx_dummy_probe(struct platform_device *pdev)
{
	...
	imx_dummy_dai.codec_of_node = codec_np;
	imx_dummy_dai.cpu_dai_name = dev_name(&cpu_pdev->dev);
	imx_dummy_dai.platform_of_node = cpu_np;
	imx_dummy_dai.playback_only = 1;
