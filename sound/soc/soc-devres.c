...
/**
 * auralic_devm_snd_soc_register_card - resource managed card registration
 * @dev: Device used to manage card
 * @card: Card to register
 *
 * Register a card with automatic unregistration when the device is
 * unregistered.
 */
int auralic_devm_snd_soc_register_card(struct device *dev, struct snd_soc_card *card,int idx)
{
	struct snd_soc_card **ptr;
	int ret;
	...
	ret = auralic_snd_soc_register_card(card,idx);
	if (ret == 0) {
		*ptr = card;
		devres_add(dev, ptr);
	} else {
...
EXPORT_SYMBOL_GPL(auralic_devm_snd_soc_register_card);
