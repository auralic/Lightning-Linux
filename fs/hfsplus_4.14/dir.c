...
static int hfsplus_symlink(struct user_namespace *mnt_userns,struct inode *dir, struct dentry *dentry,
			   const char *symname)
{
	struct hfsplus_sb_info *sbi = HFSPLUS_SB(dir->i_sb);
	struct inode *inode;
...
static int hfsplus_mknod(struct user_namespace *mnt_userns,struct inode *dir, struct dentry *dentry,
...
static int hfsplus_create(struct user_namespace *mnt_userns,struct inode *dir, struct dentry *dentry, umode_t mode,
			  bool excl)
{
	return hfsplus_mknod(&init_user_ns,dir, dentry, mode, 0);
}

static int hfsplus_mkdir(struct user_namespace *mnt_userns,struct inode *dir, struct dentry *dentry, umode_t mode)
{
	return hfsplus_mknod(&init_user_ns,dir, dentry, mode | S_IFDIR, 0);
}

static int hfsplus_rename(struct user_namespace *mnt_userns,struct inode *old_dir, struct dentry *old_dentry,
