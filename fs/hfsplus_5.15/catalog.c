		...
		const struct qstr *str, struct inode *inode)
{
	 ...
	 */
	err = hfs_bmap_reserve1(fd.tree, 2 * fd.tree->depth);
	if (err)
		goto err2;
...
int hfsplus_delete_cat(u32 cnid, struct inode *dir, const struct qstr *str)
{
	 ...
	 * have to split the root node at most once.
	 */
	err = hfs_bmap_reserve1(fd.tree, 2 * (int)fd.tree->depth - 2);
	if (err)
		goto out;
		       ...
		       struct inode *dst_dir, const struct qstr *dst_name)
{
	 ...
	 */
	err = hfs_bmap_reserve1(src_fd.tree, 4 * (int)src_fd.tree->depth - 1);
	if (err)
		goto out;

	/* find the old dir entry and read the data */
	hfsplus_cat_build_key(sb, src_fd.search_key,
	...
	hfsplus_cat_build_key(sb, dst_fd.search_key,
