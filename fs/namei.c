...
#ifdef CONFIG_AURALIC_FILELIST
bool aura_check_path_mache(int dfd, struct filename __user *name, unsigned int lookup_flags)
{
    bool ret = false;
    //struct filename *name1;
    //struct nameidata nd;
...
static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
				  ...
				  bool got_write,struct filename *pathname)
#else
static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
				  const struct open_flags *op,
				  bool got_write)
...
#ifdef CONFIG_AURALIC_FILELIST
	dentry = lookup_open(nd, file, op, got_write,nd->name);
#else
...
retry:
    ...
    {
        if(0 == error && true == aura_check_path_mache(dfd, name, 0))
        {
            info = NULL;
            while(NULL == info)
            {
	        ...
	        name1 = filename_parentat(dfd, name, 0,
				&path, &last, &type);
            if(!IS_ERR(name1))
            {
...
retry:
    ...
    if(true == vfs_can_access)
    {
        is_match = aura_check_path_mache(dfd, name, lookup_flags);
        if(true == is_match)
        {
            info = NULL;
            while(NULL == info)
...
retry:
    ...
    if(true == vfs_can_access)
    {
        match_old = aura_check_path_mache(olddfd, from, lookup_flags);
        match_new = aura_check_path_mache(newdfd, to, lookup_flags);
    }
    #endif
