...
/* epit timer */
#define	EPPHYADDR			0x302d0000
#define	EPPHYLEN			0x24
#define EPITCR				0
#define EPITSR				2
//#define EPITLR				2
#define EPITCMPR			4
...
{
	ir_stop_timer();
//    printk(KERN_ERR "in ir_recv_interrupt_handler...");	
...
enum hrtimer_restart irtimer_timeout_handle(struct hrtimer *timer)
{
	...
	/* clear idx_last */
	idx_last = 0;
	irprint(IRDBG, "timeout=%ld, idx_recv=%d!\n", ir_get_timer_cnr(), idx_recv);
...
void ir_analyse_from_txt_file(char *filename)
{
	...
	if(!IS_ERR(fp))
	{
		...
		{
			irprint(IRNOR, "auralic irkey read txt %s failed!\n", filename);
		}
		filp_close(fp, NULL);
		//set_fs(fs); 
	    force_uaccess_end(fs);
...
long irdev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	{		
		...
		if(false == is_ready_llink_up)
			wait_event_interruptible(linkwq, is_ready_llink_up);
		else
			wait_event_interruptible(linkwq, (!is_ready_llink_up));

		irprint(IRDBG, "ioctl got ready_link_up rising\n");
		break;
		
		case GET_READY_LLINK_UP_IMDIAT:
		irprint(IRDBG, "ioctl get ready_llink_up = %d\n", gpio_get_value(READY_FROM_LLINK_UP_PIN));
		int value2 = gpio_get_value(READY_FROM_LLINK_UP_PIN);
        put_user(value2,parg);
		break;
		
		case SET_PLUG_LLINK_UP:
		irprint(IRDBG, "ioctl set plug_llink_up = %lu\n", arg);
...
#define IR_PIN1  			(0*32 + 6)//22
#define IR_PIN2  			(0*32 + 5)//gpio6_03

static int lastkey = 0;
ktime_t cur, lst;
int get_time_interval( ktime_t* cur, ktime_t *lst)
{
    //return ((cur->tv_sec - lst->tv_sec)*1000 + cur->tv_usec/1000) - lst->tv_usec/1000;
    s64 delta;
    delta = ktime_ms_delta(*cur, *lst);
    return delta;
}

unsigned int get_key_value_by_step(int step, bool isright)
{
	int tmp = 0;
	switch(step)
	{
...
void update_key_by_ms(int ms, bool isright)
{
    int step = 0;
	unsigned int key;
    //printk(KERN_ERR "ms is:%d\n",ms);	
    if(ms < 3)
        step = 8;
    else if(ms < 6)
        step = 4;
	...
	key = get_key_value_by_step(step, isright);
	input_report_key(aura_input_dev, key, 1);
	input_report_key(aura_input_dev, key, 0);
	input_sync(aura_input_dev);
}

irqreturn_t rotary_interrupt_handler(int devid, void * data)
{
    int ms;
    int key = (gpio_get_value(IR_PIN1) << 1) | gpio_get_value(IR_PIN2);
	...
	if(0 == key)
	{
	    ...
	    {
	        /* right */
	        update_key_by_ms(ms, true);
        }
	    else if(1 == lastkey)
	    {
	        /* left */
	        update_key_by_ms(ms, false);
	    }
	}
...
{	
	int last_key, new_key;
	
    irprint(IRNOR, "auralic start irkey_polling_task_fn\n");
    
	//last_key = gpio_get_value(IR_BTN);
    
    while(!kthread_should_stop())
    {
        
        schedule_timeout_interruptible(HZ/100);
		//new_key = gpio_get_value(IR_BTN);        
        if(last_key != new_key)
        {
            last_key = new_key;
        	if(new_key)
        		input_report_key(aura_input_dev, KEY_F9, 0);
        	else
        		input_report_key(aura_input_dev, KEY_F9, 1);
...
{	
    ...
    irprint(IRNOR, "auralic start plug_poll_task_fn\n");
    ...
    while(!kthread_should_stop())
    {
        ...
        if(plugfromdown != new_key)
        {
            ...
            irprint(IRNOR, "interrupt  plug_llink_down = %d\n", new_key);
    		if(false == is_plug_llink_down)
        ...
        {
            readyfromup = new_key;
            irprint(IRNOR, "interrupt ready_llink_up = %d\n", new_key);					
    		if(false == is_ready_llink_up)
    ...
    irprint(IRNOR, "auralic leave plug_poll_task_fn\n");
...
void aura_input_init(int key_start_val, int key_end_val)
{
	...
	if(0 != request_irq(gpio_to_irq(IR_PIN1),
       rotary_interrupt_handler, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "irkey1", NULL))
    {
        printk("auralic irkey request rotary pin1 interrupt failed!\n");
        return; 
    }
    
	if(0 != request_irq(gpio_to_irq(IR_PIN2), 
       rotary_interrupt_handler, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "irkey2", NULL))
    {
        printk("auralic irkey request rotary pin2 interrupt failed!\n");
        return;
    }
    ...
    {
        irprint(IRNOR, "auralic PLUG_FROM_LLINK_DOWN_PIN gpio request failed!\n");
        return;
    }
    ...
    {
        irprint(IRNOR, "auralic READY_FROM_LLINK_UP_PIN gpio request failed!\n");
        return;
    }
