...
#define 	PLUG_FROM_LLINK_DOWN_PIN2    IMX_GPIO_NR(4, 13)//input //READY_FROM_LLINK_UP_PIN
#define 	READY_TO_LLINK_DOWN_PIN2       IMX_GPIO_NR(4, 15)//output //PLUG_TO_LLINK_UP_PIN

bool is_plug_llink_down = false;
bool is_plug_llink_down2 = false;
...
long irdev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	{		
        ...
        case GET_PLUG_LLINK_DOWN2_BLOCK:
		if(false == is_plug_llink_down2)
			wait_event_interruptible(linkwq, is_plug_llink_down2);
		else
			wait_event_interruptible(linkwq, (!is_plug_llink_down2));

		irprint(IRDBG, "ioctl got plug_link_down2 rising\n");
		break;
		
		case GET_PLUG_LLINK_DOWN2_IMDIAT:
		irprint(IRDBG, "ioctl get plug_llink_down2 = %d\n", gpio_get_value(PLUG_FROM_LLINK_DOWN_PIN2));
		int value2 = gpio_get_value(PLUG_FROM_LLINK_DOWN_PIN2);
        put_user(value2,parg);
		break;
        case SET_READY_LLINK_DOWN2:
		irprint(IRDBG, "ioctl set ready_llink_down2 = %lu\n", arg);
        gpio_set_value(READY_TO_LLINK_DOWN_PIN2, arg);
		break;
...
{	
    ...
    int plugfromdown, plugfromdown2;
    ...
    plugfromdown2  = gpio_get_value(PLUG_FROM_LLINK_DOWN_PIN2);
    ...
    while(!kthread_should_stop())
    {
     ...
     /*   
       ...
       */ 
		...
		new_key = gpio_get_value(PLUG_FROM_LLINK_DOWN_PIN2); 
        if(plugfromdown2 != new_key)
        {
            plugfromdown2 = new_key;
            irprint(IRNOR, "plug_llink_down_interrupt2 = %d\n", new_key);					
    		if(false == is_plug_llink_down2)
    			is_plug_llink_down2 = true;
    		else 
    			is_plug_llink_down2 = false;
...
/*    
  ...
  */  
    ...
    */
    ...
    if(0 != gpio_request(PLUG_FROM_LLINK_DOWN_PIN2,  "PLUG_FROM_LLINK_DOWN_PIN2"))
    {
        irprint(IRNOR, "auralic READY_FROM_LLINK_UP_PIN gpio request failed!\n");
        return;
    }
    gpio_direction_input(PLUG_FROM_LLINK_DOWN_PIN2);
...
*/
    ...
    gpio_direction_output(READY_TO_LLINK_DOWN_PIN2, 1);    // 1=not ready   0=ready
...
{	
    ...
    gpio_free(READY_TO_LLINK_DOWN_PIN2);
    gpio_free(PLUG_FROM_LLINK_DOWN_PIN);
    gpio_free(PLUG_FROM_LLINK_DOWN_PIN2);
