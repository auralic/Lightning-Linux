...
static irqreturn_t pca9450_irq_handler(int irq, void *data)
{
	...
	if (status & IRQ_PWRON)
		dev_err(pca9450->dev, "PWRON interrupt.\n");

	if (status & IRQ_WDOGB)
		dev_err(pca9450->dev, "WDOGB interrupt.\n");

	if (status & IRQ_VR_FLT1)
		dev_err(pca9450->dev, "VRFLT1 interrupt.\n");

	if (status & IRQ_VR_FLT2)
		dev_err(pca9450->dev, "VRFLT2 interrupt.\n");

	if (status & IRQ_LOWVSYS)
		dev_err(pca9450->dev, "LOWVSYS interrupt.\n");

	if (status & IRQ_THERM_105)
		dev_err(pca9450->dev, "IRQ_THERM_105 interrupt.\n");

	if (status & IRQ_THERM_125)
		dev_err(pca9450->dev, "IRQ_THERM_125 interrupt.\n");
