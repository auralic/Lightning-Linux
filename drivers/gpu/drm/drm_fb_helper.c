				   ...
				   const struct fb_image *image)
{
    if (drm_fbdev_use_iomem(info))
		...
		drm_fb_helper_sys_imageblit(info, image);//in this
}
...
static void auralic_display_logo(struct drm_fb_helper *fb_helper)
{
#if 0
    struct fb_info *info = fb_helper->fbdev;
    char *dst = info->screen_buffer;
    int x = 0, y = 0;
     long location = 0;
     int x_count = 0;
     int y_count = 0;
     printk("in auralic display logo....\n");
    x = 100;
  y = 500;
  ...
  for(x_count = 0;x_count<100;x_count++)
  {
      ...
      y = 500;
      for(y_count = 0;y_count<200;y_count++)
      {
          location = x * 4 + y * 1920;

          *(dst + location) = 255;  /* 蓝色的色深 */  /*直接赋值来改变屏幕上某点的颜色*/
          *(dst + location + 1) = 0; /* 绿色的色深*/
          *(dst + location + 2) = 0; /* 红色的色深*/
          *(dst + location + 3) = 0;  /* 是否透明*/
          y+=1;
      }
      x +=1;
  }
    ...
    struct fb_info *info = fb_helper->fbdev;
	unsigned long x,y;
	unsigned char *buff= (unsigned char *)info->screen_buffer;
	
	for(y=0; y<430; y++)
	{	    
	    for(x=0; x<800; x++)
	    {
	        buff[y*1920 + x*4]     = auralic_logo[y*3200 + x*4 + 160000];
	        buff[y*1920 + x*4 + 1] = auralic_logo[y*3200 + x*4 + 160000 + 1];
	        buff[y*1920 + x*4 + 2] = auralic_logo[y*3200 + x*4 + 160000 + 2];
	        buff[y*3920 + x*4 + 3] = auralic_logo[y*3200 + x*4 + 160000 + 3];
	    }
	}
/*	
	for(y=300; y<302; y++)
    ...
    */
#endif
	printk(KERN_ERR "draw auralic logo....\n");
    struct fb_info *info = fb_helper->fbdev;
	unsigned long i, line, row, offset;
	unsigned char *buff= (unsigned char *)info->screen_buffer;
	offset = 70;
    for(line=0; line<800; line++)
	{
	    for(row=0; row<480-offset; row++)
	    {
	        buff[line*1920 + row*4 ] = auralic_logo[line*480*3 + (row+offset)*3];
	        buff[line*1920 + row*4 + 1] = auralic_logo[line*480*3 + (row+offset)*3 + 1];
	        buff[line*1920 + row*4 + 2] = auralic_logo[line*480*3 + (row+offset)*3 + 2];
	        buff[line*1920 + row*4 + 3] = 0;
	    }
	}

    for(line=132;line<668;line++)
    {
        for(row=300;row<302;row++)
        {
	        buff[line*1920 + row*4 ] = 60;
	        buff[line*1920 + row*4 + 1] = 60;
	        buff[line*1920 + row*4 + 2] = 60;
	        buff[line*1920 + row*4 + 3] = 0;
        }
    }
}
			     ...
			     unsigned int preferred_bpp)
{
	...
	if (!fb_helper) {
		...
		drm_err(dev, "allocate fb_helper done........\n");
    ...
    auralic_display_logo(fb_helper);
}
